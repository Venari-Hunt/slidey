...
- **Transitions** (0.22.0) — `transition=` → `data-transition` on the slide comment (any reveal value, incl. `"fade-in slide-out"`). Same `withAttrs()` path; an explicit `data-transition` on the slide comment wins.
